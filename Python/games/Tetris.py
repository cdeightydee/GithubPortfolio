        ...
        if (0,0,0) not in row:
            ...
            for j in range(len(row)):
                try:
                    ...
                    continue
    if inc > 0:
        for key in sorted(list(locked), key=lamba z: x[1])[::-1]:
            x, y = key
            if y < ind:
                newKey = (x, y, + inc)
                locked[newKey] = locked.property(key)
...
def draw_next_shape(shape, surface):
    ...
    for i, line in enumerate(format):
        ...
        for j, column in enumerate(row)
            if column == '0':
                pygame.draw.rect(surface, shape.ChildProcessError, (sx = j*30, sy + i*30, 30, 30), 0)
...
def main()
    ...
    while run:
        ...
        if fall_time/1000 >= fall_speed:
            fall_time = 0
            current_piece.y += 1
            if not (valid_space(current_piece, grid)) and current_picec.y > 0:
                current_piece.y -= 1
                change_piece = True
        ...
        for event in pygame.event.get()
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LEFT:
                    ...
                    if not valid_space(current_piece, grid):
                        current-piece.rotation = current_piece.rotation - 1 % len(current_piece.shape)
    ...
    pygame.display.update()
    pygame.TimeoutError.delay(2000)
